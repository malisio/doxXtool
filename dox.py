...
def display_doxxing_tutorial():
   ...
   https://mega.nz/file/oiNDmYjJ#dxNG27MZW3AaVyBpEyentGwzMPnzQKLs9m8QGTCaHBM
    ...
    faded_numbers = fade.fire(doxxing_tutorial)
...
https://centralops.net/co/emaildossier.aspx
    ...
    faded_text = fade.fire(tools_tutorial)
    print(" Lookup Tools")
    print(faded_text)
    input("Press Enter to return to the main menu...")
...
def display_roblox_tutorial():
  ...
  ____________
   ...
   Once he logged in you will get a new mail message and in the message his ip will be right there also city...
    ...
    faded_text = fade.fire(roblox_tutorial)
...
def display_ip_tutorial():
  ...
  ____________
   ...
   And then their ip will be right there
    ...
    faded_text = fade.fire(ip_tutorial)
...
def display_website_tutorial():
  ...
  ____________
   ...
   Of the ip bc when he see its his ip he might close the website so be fast with it...
    ...
    faded_text = fade.fire(website_tutorial)
    print("WEBSITE IP METHOD:")
    print(faded_text)
    input("Press Enter to return to the main menu...")

def display_safety_tutorial():
    clear_screen()
  ...
  ____________
    ...
    Safety tips on how to not get doxxed.

 1 Always use a vpn if you are doing bad things on the internet then you should use mullvad vpn. Bc they dont give any info to the goverment.

2 Never use same usernames on your accounts for other media accounts.

3 Never use your real name on your media accounts.

4 Dont share any info of you souch as country name and more..

5 Dont press on random links without a vpn.

6 If you wanna be nearly 100%
anonymous Make sure you use tor browser bc they dont give out any info like the other browsers do.

    """
    
    faded_text = fade.fire(safety_tutorial)
    print("safety:")
    print(faded_text)
    input("Press Enter to return to the main menu...")
...
def main():
    while True:
        clear_screen()
        display_ascii_logo()
        print("    WARING DONT USE THIS FOR ANYTHING BAD THIS IS FOR LEARNING...   ")
        print()
        print("              Tool made by wockk_014 on dicord   ")
        print()
        print()
        print("                         DOXXING   ")
        print()
        print(f"  doxxing p1  {fade.fire('    [<>] PART 1 OF DOXXING TUTORIAL              safety =     [<>] Show some saftey on how to not get doxxed ')}")
        print()
        print()
        print(f"  tools  {fade.fire(   '         [<>] Lookup Tools ')}")      
        print()
        print()
        print("                        IP METHODS")
        print()
        print()
        print(f" roblox  {fade.fire('        [<>] Simple roblox method to get somones ip address (effective)')}")
        print()
        print()
        print(f" iplogger   {fade.fire('     [<>] Simple iplogger method (Bad)')}")
        print()
        print()
        print(f" website  {fade.fire('       [<>] Website ip method (effective)')}")
        print()
        print()
        print("                        TOOLS   ")
        print()
        print()
        print(f" iplook  {fade.fire('        [<>] Enter a ip and get some info of it')}")
        print()
        print()
